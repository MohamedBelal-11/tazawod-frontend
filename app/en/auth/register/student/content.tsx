    ...
    >
      ...
      >
        ...
        <div className="flex gap-4 flex-wrap justify-center *:flex-grow">
          ...
          <div className={classes.createDateCard}>
            <p className="text-2xl">Starts at</p>
            <input
              type="time"
            ...
            <p className={red ? "text-red-500" : undefined}>
              The number of minutes must be 30 or 0.
            </p>
          </div>
          <div className={classes.createDateCard}>
...
export default function Content() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        setMessage(["Student account has created"]);
        setLoading(false);
      ...
      setMessage([
        "An error occurred while creating the Student account: " + error,
      ]);
      setLoading(false);
    }
  };
  ...
  const submit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (name.trim() === "") {
      alive = false;
      setMessage((m) => [...m, "Please fill in the name field."]);
    }
    ...
    for (let list of [quraan_days]) {
      if (list.length === 0) {
        setMessage((m) => {
          return [
            ...m,
            "The appointment field is empty. Please enter at least one appointment.",
          ];
        });
        alive = false;
        alive1 = false;
        ...
        for (let day of dayslistd) {
          if (day === EGday) {
            setMessage((m) => {
              return [
                ...m,
                "There are two appointments on the same day according " +
                  "to Egypt time. Expand the distance between them.",
              ];
            });
            alive = false;
            alive1 = false;
        ...
        for (let dateNum of tmpList) {
          if (
            ...
            setMessage((m) => {
              return [
                ...m,
                "Appointments overlap, please check your appointments.",
              ];
            });
            break;
          }
        ...
        >
          ...
          <PasswordInput
            ...
            className={
              ...
              "rounded-xl border-solid outline-0 shadow-xl pr-12"
            }
          ...
          <h2 className="text-xl">
            Quran Memorization Lesson Schedule (at least one required)
          </h2>
          <div className={classes.lesson}>
            {quraan_days.map((date, i) => {
              return (
                <p key={quraan_days.indexOf(date)}>
                  on {date.day} at {date.starts.slice(0, -3)} duartion{" "}
